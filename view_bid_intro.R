...
multiple = sort(unlist(lapply(bid.rate.rel.diffs, function(bid.rate){ (1 + bid.rate) * (1 +intro.rate.rel.diffs)})))

print('Two distributions multiplied')
print(quantile(multiple, percentiles))